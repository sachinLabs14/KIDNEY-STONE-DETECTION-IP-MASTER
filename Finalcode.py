import cv2
import numpy as np
import matplotlib.pyplot as plt
import os

# Path setup
base_path = r'C:\Users\Arnab Sinha\Documents\GitHub\Kidney-Stone-Detection-IP\images'
image_no = r'C:\Users\LapMac\OneDrive\Desktop\Kidney-Stone-Detection-IP-master\Images Used\image1.jpg'
image_path = os.path.join(base_path, image_no)

# Load grayscale image
img = cv2.imread(image_path, 0)
if img is None:
    print(f"Error: Could not load image from {image_path}")
    exit()

def build_filters():
    filters = []
    ksize = 31
    for theta in np.arange(0, np.pi, np.pi / 32):
        params = {
            'ksize': (ksize, ksize), 'sigma': 0.0225, 'theta': theta,
            'lambd': 15.0, 'gamma': 0.01, 'psi': 0, 'ktype': cv2.CV_32F
        }
        kern = cv2.getGaborKernel(**params)
        kern /= 1.5 * kern.sum()
        filters.append((kern, params))
    return filters

def process(img, filters):
    accum = np.zeros_like(img)
    for kern, _ in filters:
        fimg = cv2.filter2D(img, cv2.CV_8UC3, kern)
        np.maximum(accum, fimg, accum)
    return accum

def Histeq(img):
    return cv2.equalizeHist(img)

def GaborFilter(img):
    filters = build_filters()
    return process(img, filters)

def Laplacian(img, par):  
    lap = cv2.Laplacian(img, cv2.CV_64F)
    sharp = img - par * lap
    sharp = np.uint8(cv2.normalize(sharp, None, 0, 255, cv2.NORM_MINMAX))
    return sharp

def Watershed(img):
    ret, thresh = cv2.threshold(img, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)

    kernel = np.ones((3, 3), np.uint8)
    opening = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, kernel, iterations=2)
    sure_bg = cv2.dilate(opening, kernel, iterations=3)

    dist_transform = cv2.distanceTransform(opening, cv2.DIST_L2, 5)
    ret, sure_fg = cv2.threshold(dist_transform, 0.23 * dist_transform.max(), 255, 0)

    sure_fg = np.uint8(sure_fg)
    unknown = cv2.subtract(sure_bg, sure_fg)

    ret, markers = cv2.connectedComponents(sure_fg)
    markers = markers + 1
    markers[unknown == 255] = 0

    img2 = cv2.imread(image_path)
    if img2 is None:
        print(f"Error: Could not reload image for watershed: {image_path}")
        exit()

    img2 = cv2.medianBlur(img2, 5)
    markers = cv2.watershed(img2, markers)
    img2[markers == -1] = [255, 0, 0]

    return img2

# Apply the appropriate filter
if image_no == 'image1.jpg':
    img3 = Laplacian(img, 0.239)
elif image_no == 'image2.jpg':
    img3 = Histeq(GaborFilter(img))
elif image_no == 'image4.jpg':
    img3 = GaborFilter(img)
else:
    img3 = img  # Default fallback

# Apply watershed segmentation
img3 = Watershed(img3)

# Convert to RGB for correct color display in matplotlib
if len(img3.shape) == 3:
    img3 = cv2.cvtColor(img3, cv2.COLOR_BGR2RGB)

# Display final result
plt.imshow(img3)
plt.title('Marked')
plt.xticks([]), plt.yticks([])
plt.show()
